import userDb from "../cms/infrastructure/database/user/user.db-repository";
import type usersModel = require("../cms/domain/user/userModal");
import { Iuser } from "../core/database/Entites/interfaceUser";
...
export const getAllUsers: usersModel.GetAllUsers = async () => {
  try {
    ...
    const arrByModelUser: usersModel.User[] = arrUserObjDb.map((user) => {
      return {
        email: user.email,
        password: user.pass,
...
export const addUser: usersModel.AddUser = async (userObj: usersModel.User) => {
  try {
    ...
    if (iuser !== undefined) {
      const userObjModel: usersModel.User = {
        email: iuser.email,
        password: iuser.pass,
...
export const deleteUserWithEmail: usersModel.DeleteUser = async (email?: usersModel.User["email"] | undefined) => {
  if (email === undefined) return "the email is undefined cant delete";
...
const updateUserNameWithEmail: usersModel.UpdateUser = async (userObj: usersModel.User) => {
  ...
  userDb
    ...
    .then((getUser): usersModel.UpdateResult | undefined => {
      if (!getUser) return "this user dont exist in db";
      userDb
        .updateUserByEmail(userObj)
        .then((): usersModel.UpdateResult => {
